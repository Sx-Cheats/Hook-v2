...
struct EnumObj
{
    ...
    EnumObj(T e=NULL, EnumObj<T>* n=nullptr) : element(e), Next(n) {};
};


typedef struct __QUERY_WINDOWS_SERVICES
{
public:

    char *  ServiceName;

    LPSERVICE_STATUS_PROCESS  NEnumServiceStatutsProcess;
    LPQUERY_SERVICE_CONFIGA   NQueryServiceConfig;

    __QUERY_WINDOWS_SERVICES(LPSTR servicename,LPSERVICE_STATUS_PROCESS enumservicestatutprocess, LPQUERY_SERVICE_CONFIGA nqueryserviceconfig) : NEnumServiceStatutsProcess(enumservicestatutprocess), NQueryServiceConfig(nqueryserviceconfig) 
    {
        ServiceName = (char *)calloc(1,strlen(servicename));
    ...
    {
        free(ServiceName);
        free(NEnumServiceStatutsProcess);
        free(NQueryServiceConfig);

    }

} QUERY_WINDOWS_SERVICES, * PQUERY_WINDOWS_SERVICES;

#define SERVICES_WINDOWS std::vector<PQUERY_WINDOWS_SERVICES>
...
public:
    ...
    static PSYSTEM_PROCESS_INFORMATION CurrentProcess;
    static void NtMemCpy(BYTE* dst, BYTE* src, size_t size);
